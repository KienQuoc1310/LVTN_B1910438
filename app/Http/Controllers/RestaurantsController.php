...
class RestaurantsController extends Controller
{
    ...
    public function restaurants_near($slug)    
    {     
          $restaurant = Restaurants::where("restaurant_slug", $slug)->first();

        //   dd($restaurant);

          return view('pages.restaurants_near',compact('restaurant'));
    ...
    public function restaurants_distance(Request $request)    {
        $inputs = $request->all();

        $user_id=Auth::user()->id;

        $restaurants = Post::find(1);

        $restaurants->restaurant_distance = $inputs['restaurant_distance']; 

        $restaurants->update();

        return view('pages.restaurants',compact('restaurant_distance'));
    }
